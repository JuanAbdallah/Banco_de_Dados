...
select pnome, nome_dependente
from funcionario join dependente on cpf = fcpf;

select pnome, unome, dnome
from funcionario join departamento on dnumero = dnr;

select dnome,pnome,unome
from funcionario join departamento on cpf = cpf_gerente;

select pnome,unome,projnome
from funcionario join trabalha_em on cpf = fcpf join projeto on pnr = projnumero
where horas > 20;
